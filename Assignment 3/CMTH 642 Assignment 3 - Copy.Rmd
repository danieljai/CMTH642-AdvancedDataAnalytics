...
<center>  <h3> Andy Lee </h3> </center>
<center> <h3> DHC 500163559 </h3> </center>
---
...
df.normalized$qualitylevels <- df$qualitylevels
```

#### 8. Divide the dataset to training and test sets. (12 points)
```{r}
set.seed(888)
train_index <- sample(1:nrow(df.normalized), 0.8*nrow(df.normalized))
df.train <- df.normalized[train_index,]
df.test <- df.normalized[-train_index,]
```
...
library(class)
library(gmodels)
df.knn <- knn(train = df.train[,-12], test = df.test[,-12], cl=df.train$qualitylevels, k=13)
```
